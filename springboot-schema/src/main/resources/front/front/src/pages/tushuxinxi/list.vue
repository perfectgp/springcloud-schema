...
<div>
	...
	<div class="list-preview" :style='{"width":"1200px","margin":"10px auto","position":"relative","flexWrap":"wrap","background":"none","display":"flex"}'>
		...
		<div class="category-2" :style='{"top":"0","left":"-110px","flexDirection":"column","display":"flex","width":"94px","position":"absolute","height":"auto","zIndex":"9"}'>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"border":"2px solid #a4d9b7","padding":"10px 0","margin":"10px 0 0 0","borderRadius":"8px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"margin":"10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","color":"#29292a","display":"inline-block"}'>图书名称</div>
        <el-input v-model="formSearch.tushumingcheng" placeholder="图书名称" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","color":"#29292a","display":"inline-block"}'>图书类型</div>
        <el-select v-model="formSearch.tushuleixing" placeholder="请选择图书类型" :clearable="true">
          <el-option v-for="(item, index) in tushuleixingOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","color":"#29292a","display":"inline-block"}'>作者</div>
        <el-input v-model="formSearch.zuozhe" placeholder="作者" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('tushuxinxi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"10px","outline":"none","color":"#fff","borderRadius":"10px","background":"#55b44b","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/tushuxinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"margin":"20px 0 0 0","flex":"1"}'>
		...
		<div class="list2 index-pv1" :style='{"border":"2px solid #a4d9b7","padding":"20px 0px 0 0px","overflow":"hidden","borderRadius":"8px","background":"#fff","width":"100%","height":"auto"}'>
			<div :style='{"cursor":"pointer","margin":"0 0 20px 2%","borderRadius":"10px","flexWrap":"wrap","background":"#fff","display":"flex","width":"48%","fontSize":"0","float":"left","height":"150px"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"boxShadow":"8px 0px 0 rgba(146, 181, 164,.5)","borderColor":"rgba(146, 181, 164,.8)","objectFit":"cover","borderRadius":"10px 40px 40px 10px","borderWidth":"0 12px 0 0","width":"240px","borderStyle":"solid","float":"left","height":"100%"}' v-if="item.tushufengmian && item.tushufengmian.substr(0,4)=='http'" :src="item.tushufengmian" class="image" />
				<img :style='{"boxShadow":"8px 0px 0 rgba(146, 181, 164,.5)","borderColor":"rgba(146, 181, 164,.8)","objectFit":"cover","borderRadius":"10px 40px 40px 10px","borderWidth":"0 12px 0 0","width":"240px","borderStyle":"solid","float":"left","height":"100%"}' v-else :src="baseUrl + (item.tushufengmian?item.tushufengmian.split(',')[0]:'')" class="image" />
				<div class="item-info" :style='{"padding":"12px","margin":"0 20px 0 40px","overflow":"hidden","borderRadius":"8px","background":"#eff8f2","width":"calc(100% - 300px)","position":"relative","float":"right","height":"auto"}'>
					<div :style='{"width":"100%","margin":"0px","lineHeight":"32px","fontSize":"16px","color":"#333","background":"none"}' class="name ">{{item.tushumingcheng}}</div>
					<div :style='{"width":"100%","margin":"0px","lineHeight":"32px","fontSize":"16px","color":"#333","background":"none"}' class="name ">{{item.tushuleixing}}</div>
					<div :style='{"width":"100%","margin":"0px","lineHeight":"32px","fontSize":"16px","color":"#333","background":"none"}' class="name ">{{item.zuozhe}}</div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/tushuleixing/tushuleixing').then(res => {
        if (res.data.code == 0) {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.tushuleixing != '') searchWhere.tushuleixing = this.formSearch.tushuleixing;
        if (this.formSearch.zuozhe != '') searchWhere.zuozhe = '%' + this.formSearch.zuozhe + '%';
        if (this.curFenlei != '全部') searchWhere.tushuleixing = this.curFenlei;
