<template>
	<div class="addEdit-block" :style='{"padding":"30px","backgroundAttachment":"fixed","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20230312/52c1bcab43064c72aa492e0f342d2666.jpg)","flexDirection":"row-reverse","display":"flex","backgroundSize":"cover","backgroundPosition":"center center","backgroundRepeat":"no-repeat","height":"calc(100vh - 120px)"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="章节数" prop="chapternum">
					<el-input v-model="ruleForm.chapternum" placeholder="章节数" clearable  :readonly="ro.chapternum"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="章节数" prop="chapternum">
					<el-input v-model="ruleForm.chapternum" placeholder="章节数" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="章节标题" prop="chaptertitle">
					<el-input v-model="ruleForm.chaptertitle" placeholder="章节标题" clearable  :readonly="ro.chaptertitle"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="章节标题" prop="chaptertitle">
					<el-input v-model="ruleForm.chaptertitle" placeholder="章节标题" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="章节内容" prop="content">
					<editor 
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else-if="ruleForm.content" label="章节内容" prop="content">
...
export default {
	data() {
		...
		return {
			...
			ruleForm: {
                refid: this.$storage.get("chapterRefid"),
				chapternum: '',
				chaptertitle: '',
				content: '',
			},
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.content = this.ruleForm.content.replace(reg,'../../../springcloudha1ml/upload');
        } else {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `chaptertushuxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.chaptertushuxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
